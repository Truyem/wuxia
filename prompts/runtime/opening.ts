...
3. Khởi tạo toàn bộ GameState qua \`tavern_commands\`:
   - gameState.Character: Theo hồ sơ (Chỉ số, Sinh tồn, Trang bị, Túi đồ, Công pháp, Tiền).
     - Chỉ số HP 7 bộ phận (BẮT BUỘC): Khởi tạo TRỰC TIẾP trong gameState.Character. Các khóa: headCurrentHp, headMaxHp, headStatus, chestCurrentHp, chestMaxHp, chestStatus, abdomenCurrentHp, abdomenMaxHp, abdomenStatus, leftArmCurrentHp, leftArmMaxHp, leftArmStatus, rightArmCurrentHp, rightArmMaxHp, rightArmStatus, leftLegCurrentHp, leftLegMaxHp, leftLegStatus, rightLegCurrentHp, rightLegMaxHp, rightLegStatus. (VD: "headCurrentHp": 100, "headMaxHp": 100, "headStatus": "Khỏe mạnh"). TUYỆT ĐỐI KHÔNG dùng sub-key trung gian như bodyPartHP.
   ...
   - gameState.World: NPC năng động, Bản đồ, Kiến trúc, Sự kiện.
     ...
     - ongoingEvents: title, content, location, startTime, estimatedEndTime (YYYY:MM:DD:HH:MM).
