    ...
    const [options, setOptions] = useState<ModelOptions>(() => {
        ...
        try {
            if (saved) {
                ...
                // Handle legacy 'claud' key if present in localStorage
                if (parsed.claud && !parsed.claude) {
                    parsed.claude = parsed.claud;
                    delete parsed.claud;
                }
                return { ...DEFAULT_OPTIONS, ...parsed };
            }
        } catch (e) {
            console.warn('Failed to parse saved model options', e);
        }
            ...
            if (resultModels.length > 0) {
                const newOptions: ModelOptions = {
                    ...
                    other: resultModels.filter(m => 
                        ...
                        !DEFAULT_OPTIONS.claude.includes(m) && 
                        !DEFAULT_OPTIONS.deepseek.includes(m) &&
                        !DEFAULT_OPTIONS.mistral.includes(m) &&
                        !DEFAULT_OPTIONS.groq.includes(m)
                    )
                };
                setOptions(newOptions);
                localStorage.setItem(STORAGE_KEY, JSON.stringify(newOptions));
