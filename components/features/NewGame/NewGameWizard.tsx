                            ...
                            <OrnateBorder className="p-4 md:p-8">
                                ...
                                <div className="space-y-6">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        {worldConfig.storyStyle === 'NTL Hậu cung' && (
                                            <div className="space-y-2 group animate-fade-in">
                                                <label className="text-sm font-medium text-wuxia-gold group-hover:text-amber-300 transition-colors block italic">Bậc NTL Hậu cung</label>
                                                <InlineSelect
                                                    value={worldConfig.ntlHaremTier || 'Cấm loạn luân'}
                                                    options={NTL_HAREM_TIER_OPTIONS}
                                                    onChange={(val) => setWorldConfig(prev => ({ ...prev, ntlHaremTier: val as NTLHaremTier }))}
                                                    className="w-full"
                                                />
                                            </div>
                                        )}

                                        <div className="md:col-span-2 space-y-4 pt-2 border-t border-wuxia-gold/10 mt-2">
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                <div className="flex items-center justify-between group p-3 bg-wuxia-gold/5 rounded-lg border border-wuxia-gold/10 hover:border-wuxia-gold/30 transition-all">
