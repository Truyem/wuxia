            ...
            <div className="bg-modal-gradient border border-wuxia-gold/30 w-full max-w-[620px] h-[86vh] flex flex-col shadow-[0_0_60px_rgba(0,0,0,0.8)] relative overflow-hidden rounded-2xl">
                <div className="h-12 shrink-0 border-b border-gray-800/60 bg-ink-black/40 flex items-center justify-between px-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <div className="text-wuxia-gold font-serif font-bold text-base">{displayName}</div>
                            <div className="text-[9px] text-gray-500 font-mono">{hasJoinedSect ? sectData.playerPosition : 'Chưa gia nhập'}</div>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar p-4 space-y-4">
                    {activeTab === 'hall' && (
                        <>
                            {hasJoinedSect ? (
                                ...
                                <div className="bg-ink-black/40 border border-amber-900/30 rounded-xl p-6 text-center">
                                    <div className="text-xl mb-2 text-amber-400">Chưa gia nhập môn phái</div>
                                    <p className="text-sm text-gray-400">Hãy tìm một môn phái phù hợp để gia nhập. Sau khi gia nhập, bạn sẽ có thể nhận nhiệm vụ, đổi điểm cống hiến và xem thông tin môn phái.</p>
                                </div>
                            )}
                        </>
                    )}
                    ...
                    {activeTab === 'missions' && (
                        <>
                            {!hasJoinedSect ? (
                                <div className="bg-ink-black/40 border border-gray-800 rounded-xl p-6 text-center">
                                    <p className="text-sm text-gray-500">Cần gia nhập môn phái để nhận nhiệm vụ</p>
                                </div>
                            ) : (
                                <>
                                    <div className="flex gap-2">
                                        {['all', 'available', 'active'].map(f => (
                                            <button
                                                ...
                                                className={`px-3 py-1.5 text-[11px] rounded-full border transition-all ${
                                                    missionFilter === f 
                                                        ? 'bg-wuxia-gold/15 border-wuxia-gold text-wuxia-gold' 
                    ...
                    {activeTab === 'exchange' && (
                        <>
                            {!hasJoinedSect ? (
                                <div className="bg-ink-black/40 border border-gray-800 rounded-xl p-6 text-center">
                                    <p className="text-sm text-gray-500">Cần gia nhập môn phái để đổi vật phẩm</p>
                                </div>
                            ) : (
                                <div className="grid grid-cols-2 gap-3">
                                    {sectData.exchangeList.map(good => (
                                        <div key={good.id} className="bg-ink-black/40 border border-gray-800 rounded-xl p-3 space-y-2">
                                            ...
                                            <div className="text-[10px] text-gray-500">Yêu cầu {good.requiredPosition}</div>
                                            <div className="flex items-center justify-between text-[10px]">
                                                <span className="text-wuxia-gold font-mono">{good.exchangePrice} Contribution</span>
                                                <span className="text-gray-500">Kho {good.stock}</span>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                    ...
                    {activeTab === 'members' && (
                        <>
                            {!hasJoinedSect ? (
                                <div className="bg-ink-black/40 border border-gray-800 rounded-xl p-6 text-center">
                                    <p className="text-sm text-gray-500">Cần gia nhập môn phái để xem thành viên</p>
                                </div>
                            ) : (
                                <div className="space-y-3">
                                    {sectData.importantMembers.map(mem => (
                                        <div key={mem.id} className="bg-ink-black/40 border border-gray-800 rounded-xl p-4 flex items-start gap-3">
                                            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-base border shrink-0 ${
                                                mem.gender === 'Nữ' 
                                                    ? 'border-pink-900 bg-pink-900/10 text-pink-500' 
                                                    : 'border-wuxia-gold/30 bg-wuxia-gold/10 text-wuxia-gold'
                                            }`}>
                                                {mem.name[0]}
                                            </div>
                                            <div className="flex-1">
                                                ...
                                                <div className="text-[10px] text-gray-500 mt-1">{mem.gender} · {mem.age}Tuổi · {mem.realm}</div>
