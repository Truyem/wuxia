                ...
                <div className="h-20 shrink-0 border-b border-wuxia-gold/10 bg-black/60 flex items-center justify-between px-10 relative z-50">
                    <div className="flex flex-col">
                        <h3 className="text-wuxia-gold font-serif font-black text-2xl tracking-[0.5em] drop-shadow-2xl">
                            {t('social.sections.title') || "DANH HIỆP PHẢ"}
                        </h3>
                        <span className="text-[9px] text-paper-white/30 uppercase tracking-[0.6em] font-bold mt-1">{t('social.sections.subtitle') || "Giang Hồ Nhân Mạch"}</span>
                    </div>
                ...
                <div className="flex-1 flex overflow-hidden">
                    ...
                    <div className="w-1/4 border-r border-wuxia-gold/10 flex flex-col bg-black/40 overflow-hidden">
                        <div className="p-4 border-b border-white/5 bg-black/20">
                            <span className="text-[10px] uppercase tracking-widest text-paper-white/30 font-bold">{t('social.labels.relation') || "Mới Quan Hệ"} ({socialList.length})</span>
                        </div>
                                        ...
                                        <div className="flex-1">
                                            <div className="flex items-center gap-4 mb-3">
                                                ...
                                                {currentNPC.isMainCharacter && (
                                                    <span className="bg-wuxia-gold text-black text-[9px] font-black px-3 py-1 rounded-none uppercase tracking-widest shadow-lg shrink-0">{t('social.badges.important') || "Quan Trọng"}</span>
                                                )}
                                            </div>
                                            
                                            <div className="flex flex-wrap gap-3">
                                                <span className="text-[10px] font-bold bg-white/5 border border-white/10 px-3 py-1 rounded-none text-paper-white/60 tracking-widest uppercase">{tValue(currentNPC.identity)}</span>
                                                <span className="text-[10px] font-bold bg-white/5 border border-white/10 px-3 py-1 rounded-none text-paper-white/60 tracking-widest uppercase">{tValue(currentNPC.realm)}</span>
                                            ...
                                            >
                                                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-paper-white/40 group-hover/toggle:text-paper-white/70">{t('social.labels.trackedStatus') || t('social.status.mainStatus') || "Theo Dõi Đặc Biệt"}</span>
                                         ...
                                         <div className="bg-black/80 p-6 rounded-none border border-wuxia-red/20 flex flex-col items-center min-w-[140px] shadow-2xl backdrop-blur-md border-b-4 border-b-wuxia-red/40 drop-shadow-md">
                                            ...
                                            <div className="text-[8px] text-paper-white/40 tracking-[0.3em] font-black uppercase text-center">{tValue(currentNPC.relationStatus)}</div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
                                    <div className={`lg:col-span-2 p-8 rounded-none border border-white/5 relative group/bio overflow-hidden ${currentNPC.gender === 'Female' || currentNPC.gender === 'Nữ' ? 'bg-pink-900/10' : 'bg-black/40'}`}>
                                        ...
                                        <div className="flex items-center gap-3 mb-6">
                                            ...
                                            <h4 className="text-wuxia-gold font-serif font-black uppercase tracking-widest text-sm">{t('social.sections.biography') || "Hành Trình Ký Sự"}</h4>
                                        </div>
                                        <p className="text-paper-white/80 font-serif leading-relaxed text-lg italic indent-8">
                                            {currentNPC.description || t('social.labels.noBiography') || "Cuốn sổ này vẫn chưa ghi lại nhiều về cuộc đời người này tại nhân gian."}
                                        </p>
                                    ...
                                    <div className="space-y-10">
                                        ...
                                        <div className="glass-ink p-8 rounded-none border border-pink-500/10 shadow-2xl relative overflow-hidden">
                                            ...
                                            <div className="flex items-center justify-between mb-8 border-b border-white/5 pb-4">
                                                <h4 className="text-pink-300 font-serif font-black uppercase tracking-widest text-sm flex items-center gap-3">
                                                    <IconGlyph name="network" className="w-4 h-4" /> {t('social.sections.socialTies') || "Mạng Lưới Quan Hệ"}
                                                </h4>
                                            ...
                                            {currentRelationshipNetwork.length > 0 ? (
                                                        ...
                                                        <div key={idx} className="bg-white/5 p-4 rounded-none border border-white/5 group/net">
                                                            <div className="flex items-center gap-3 mb-2">
                                                                ...
                                                                <div className="flex flex-col">
                                                                    ...
                                                                    <span className="text-[9px] text-pink-500/60 uppercase font-bold tracking-widest">{tValue(edge.relation)}</span>
                                                                </div>
                                                            </div>
                                                ...
                                                <div className="text-center py-10 opacity-30 italic text-sm text-paper-white/40">{t('social.labels.noConnections') || "Không rõ mối liên hệ xung quanh."}</div>
                                            )}
                                        </div>
                                        ...
                                        <div className="border border-pink-500/30 bg-black/80 rounded-none overflow-hidden shadow-2xl p-10 relative">
                                            ...
                                            <div className="flex items-center justify-between mb-10">
                                                <div className="flex flex-col">
                                                     <h4 className="text-pink-400 font-serif font-black text-2xl tracking-[0.3em] flex items-center gap-4">
                                                         {t('social.sections.privateBoudoir') || "THUẦN ÂM KHUÊ PHÒNG"}
                                                         {currentNPC.isVirgin && <span className="text-[8px] bg-pink-500/10 border border-pink-500/50 text-pink-400 px-3 py-1 rounded-none uppercase tracking-widest font-black">{t('social.labels.pureSoul') || "Xử Nữ"}</span>}
                                                     </h4>
                                                 ...
                                                 <div className="mb-10 bg-gradient-to-r from-pink-950/40 via-pink-900/10 to-transparent border border-pink-500/40 p-6 rounded-none relative overflow-hidden">
                                                    ...
                                                    <div className="relative z-10">
                                                        ...
                                                        <div className="font-serif text-lg leading-relaxed text-pink-100 italic">
                                                            Vào ngày <span className="text-wuxia-gold font-black not-italic px-1.5">{currentNPC.firstNightTime}</span>, nàng {t('social.labels.dedication') || "đã giao phó sự trong trắng của đời mình cho"} <span className="text-wuxia-gold font-black underline underline-offset-4 decoration-wuxia-gold/40 not-italic px-1">{currentNPC.firstNightClaimer}</span>.
                                                        </div>
                                            ...
                                            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
                                                <PrivateTag label={t('social.labels.chest') || "Kích Cỡ Ngực"} value={currentNPC.breastSize} color="text-pink-200" />
                                                <PrivateTag label={t('social.labels.hip') || "Kích Cỡ Mông"} value={currentNPC.buttockSize} color="text-pink-200" />
                                                <PrivateTag label={t('social.labels.nippleColor') || "Sắc Đào Đỉnh"} value={currentNPC.nippleColor} color="text-pink-300" />
                                                <PrivateTag label={t('social.labels.secretPoint') || "Sắc Động Đào"} value={currentNPC.vaginaColor} color="text-pink-300" />
                                            </div>

                                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
                                                <div className="bg-black/60 p-6 rounded-none border border-white/5 h-full">
                                                    <span className="text-[9px] font-black uppercase tracking-widest text-pink-500/60 mb-3 block">{t('social.labels.innateLust') || "Đặc Điểm Cơ Thể"}</span>
                                                    <p className="text-sm font-serif leading-loose text-pink-100/80 italic indent-4">{currentNPC.privateTraits || t('social.labels.noLustRecords') || "Bạch khiết vô hạ, không tỳ vết."}</p>
                                                </div>
                                                <div className="bg-black/60 p-6 rounded-none border border-white/5 h-full">
                                            ...
                                            <div className="bg-white/5 rounded-none border border-white/5 p-8 relative overflow-hidden">
                                                ...
                                                <div className="absolute bottom-0 right-0 w-2 h-2 border-b-2 border-r-2 border-pink-500/30"></div>
                                                 ...
                                                 <h5 className="text-[10px] font-black uppercase tracking-[0.4em] text-paper-white/30 mb-6 flex justify-between">
                                                     {t('social.sections.bodyDevelopment') || "CƠ THỂ PHÁT TRIỂN TIẾN ĐỘ"}
                                                     <span className="text-pink-500/50">LÂM SÀNG BẢNG</span>
                                                 </h5>
                                                
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12">
                                                     <SexStat label={t('social.labels.mouth') || "Duyệt nhân thủ sắc (Mouth)"} count={currentNPC.count_oral} />
                                                     <SexStat label={t('social.labels.vaginal') || "Giao long quá huyệt (Vaginal)"} count={currentNPC.count_vaginal} />
                                                     <SexStat label={t('social.labels.breast') || "Cung nghênh tuyết đỉnh (Breast)"} count={currentNPC.count_breast} />
                                                     <SexStat label={t('social.labels.anal') || "Khai khẩn hậu viên (Anal)"} count={currentNPC.count_anal} />
                                                    <div className="md:col-span-2 mt-4 flex items-center justify-center gap-10 bg-black/40 py-3 rounded-none border border-pink-500/10">
                                                        ...
                                                        <div className="w-[1px] h-8 bg-white/10"></div>
                                                         <div className="flex flex-col items-center">
                                                             <span className="text-[8px] text-pink-500/50 font-bold uppercase mb-1">{t('social.labels.climax') || "Cực khoái tột độ"}</span>
