...
const providerOptions: ApiProviderType[] = ['worker', 'gemini', 'claude', 'openai', 'deepseek', 'openai_compatible'];
    ...
    return (
        <div className="space-y-6 text-sm animate-fadeIn">
            ...
            {testResultModal.open && (
                <div
                    className="fixed inset-0 z-[260] flex items-center justify-center bg-ink-black/70 backdrop-blur-sm p-4"
                    onClick={() => setTestResultModal(prev => ({ ...prev, open: false }))}
                >
                    <div
                        className="w-full max-w-3xl rounded-lg border border-wuxia-gold/30 bg-ink-black/90 p-5 shadow-[0_0_30px_rgba(0,0,0,0.8)]"
                        onClick={(e) => e.stopPropagation()}
                    >
                        <div className="flex items-center justify-between gap-4 mb-4">
                            <h4 className={`text-lg font-serif font-bold ${testResultModal.ok ? 'text-wuxia-gold' : 'text-wuxia-gold'}`}>
