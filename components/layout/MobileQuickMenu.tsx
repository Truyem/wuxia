...
const MobileQuickMenu: React.FC<Props> = ({ activeWindow, onMenuClick, characterData }) => {
    ...
    return (
        <div className="md:hidden border-t border-wuxia-gold/20 bg-gradient-to-t from-ink-black via-ink-black/95 to-ink-black/80 backdrop-blur-sm shadow-[0_-12px_24px_rgba(0,0,0,0.38)] pb-2 flex flex-col">
            <div className="px-2 pt-1.5 overflow-x-auto">
                <div className="relative rounded-2xl border border-gray-800 /40 overflow-hidden min-w-max">
            ...
            {showAllMenus && (
                <div className="px-2 pt-2 pb-1">
                    <div className="rounded-2xl border border-gray-800 /50 shadow-[0_8px_20px_rgba(0,0,0,0.4)] overflow-hidden">
                        <div className="px-3 py-2 border-b border-gray-800 flex items-center justify-between">
                            <span className="text-[10px] tracking-[0.18em] text-gray-500">Toàn bộ chức năng</span>
...
const QuickButton = ({
    ...
    <button
        type="button"
        onClick={onClick}
        className={`relative h-14 w-16 flex flex-col items-center justify-center gap-1 transition-colors ${active ? 'text-wuxia-gold' : 'text-gray-400 hover:text-gray-100'
            }`}
    >
...
const MenuTile = ({
    ...
    <button
        ...
        className={`h-16 border rounded-xl flex flex-col items-center justify-center gap-1.5 transition-colors ${active
            ...
            : 'border-gray-800 /20 text-gray-300 hover:border-wuxia-cyan/60 hover:text-paper-white'
            }`}
    >
        <span className={`h-8 w-8 flex items-center justify-center translate-y-0.5 transition-all ${active ? 'scale-110 drop-shadow-[0_0_8px_rgba(230,200,110,0.45)]' : ''
            }`}>
